      ...
      {path==="create"&&<>
        ...
        {step===1?<>
          ...
          <div className="family-start-options">
            <button className="family-start-card recommended" disabled={busy||!canSetup} onClick={()=>setShowImport(true)}><span className="start-icon"><FileSpreadsheet/></span><span className="recommended-pill">Best for an existing family list</span><strong>Upload Excel or CSV</strong><small>Start with names and only the details you know. Relationships can be included now or added later in the app.</small><b>Open Excel guide <ArrowRight size={15}/></b></button>
            <button className="family-start-card" disabled={busy||!canSetup} onClick={()=>create("empty")}><span className="start-icon"><UsersRound/></span><strong>Start with a few relatives</strong><small>Create the family immediately with only its name, then add yourself and relatives gradually.</small><b>{approvalRequired?"Request family":"Create family now"} <ArrowRight size={15}/></b></button>
          </div>
        </>}
      </>}
