...
import {ArrowRight,Heart,Plus,UserRound} from "lucide-react";
...
export default function QuickFamilyStart({viewer,suggestedName="",onAddMyself,onAddRelative,onImport,onDismiss}:{viewer?:Member;suggestedName?:string;onAddMyself:(name:string,gender:Member["gender"])=>Promise<void>;onAddRelative:(name:string,relationship:string,gender:Member["gender"])=>Promise<void>;onImport:()=>void;onDismiss:()=>void}){
 ...
 return <section className="card quick-family-start">
   ...
   <div className="quick-family-start-alt"><button className="text-action" onClick={onImport}>Have a list? Use Excel / CSV</button><button className="text-action" onClick={onDismiss}>I’ll do this later</button></div>
