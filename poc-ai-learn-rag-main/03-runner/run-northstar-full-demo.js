...
const KIT=path.resolve(process.env.DEMO_KIT_DIR||'../G9.1-Northstar-Organization-Intelligence-Demo-Kit');
...
function resolve(a,map){a=JSON.parse(JSON.stringify(a));let s=map.get(norm(a.subject?.label)),o=map.get(norm(a.object?.label));if(a.predicate==='skill'){if(!s)return null;a.subject.entityId=s;a.object.value=a.object.value||a.object.label;return a}if(a.predicate==='owns'||a.predicate==='depends_on'){if(!s||!o)return null;a.subject.entityId=s;a.object.entityId=o;return a}return a.predicate==='architectural_decision'?a:null}
(async()=>{
 ...
 console.log('6/7 resolve labels');let resolved=ex.assertions.map(a=>resolve(a,map)).filter(Boolean);console.log('resolved',resolved.length,'of',ex.assertions.length);
