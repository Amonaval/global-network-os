...
3. **Marcus** — Law firm partner querying CASE FILES (attorney-client privilege)
...
- "Shadow Workspace" / code prediction patterns — Cursor territory
- Code generation quality improvements — not our moat
- Positioning as competitor to GitHub Copilot/Cursor for engineers — wrong category
...
|---|---|---|
| **Contextual RAG** (Anthropic) | Critical | A CA's note "client increased by 12%" is meaningless without context — identical to the chunk isolation problem Anthropic solved. 67% failure reduction. Wraps existing system. |
| **Confidence Gate / CRAG-style grading** | Already implemented | Existing dual-threshold IS CRAG. Must never be loosened — for regulated professionals, wrong answer = professional liability event. |
| **Cross-encoder reranking** | High (next improvement) | After BM25+dense+RRF retrieves 20-50 candidates, reranking to 5-10 reduces noise before LLM context. Critical for precision-over-recall mandate. |
| **Multimodal RAG** | High (future moat) | Medical (imaging, charts), Legal (PDF tables, exhibits), CA (financial statements, tax forms), Research (figures, diagrams). No local-first competitor does this. |
| **Document-type chunking** | High | Different professional documents need different strategies: hierarchical/parent-child for legal briefs; semantic for research papers; recursive-overlap for medical notes. NOT AST. |
| **RAGAS evaluation** | High | Faithfulness + Context Precision map to the confidence gate philosophy. IDS metric should eventually incorporate these. Makes every future change measurable. |
| **Hybrid retrieval (BM25+Dense+RRF)** | Already implemented | Correct. Never replace with single-method. |
| **HyDE (selective)** | Medium | Useful when CAs/consultants ask vague recall questions. Risky given confidence gate philosophy — hallucinated hypothesis must not weaken gate. |
| **GraphRAG** | Low (Stage 3+) | Legal docs (cases/parties/judges/precedents) and medical notes have dense entity graphs. Not for v1 — premature. Note for enterprise legal/medical expansion. |
| **Agentic RAG / Multi-hop** | Low (future) | "What strategy worked for clients with this profile AND how did the 2022 regulation affect it?" is multi-hop. Future capability after single-hop is irreplaceable. |
...
Content:
- What KH is (private professional document intelligence, not code intelligence)
- Relevant RAG patterns with rationale mapped to actual customer segments
- Explicitly out-of-scope patterns with rationale
- Current implementation status vs. roadmap
- Evolution path: current → contextual retrieval → reranking → multimodal

### 2. UPDATE: `.claude/.engineering/DECISIONS.md`
Add four new decisions after existing D.11:
- **D.12**: Contextual chunk enrichment adopted — why (67% failure reduction, wraps existing system, no retraining, directly solves professional document chunk isolation problem)
...
- **D.14**: Document-type chunking strategy by professional segment — why different types need different strategies (NOT AST-based)
- **D.15**: Code-aware RAG, AST chunking, and code graph traversal are explicitly out of scope — why (Cursor/Copilot territory; engineering beachhead is a DOCUMENT problem not code traversal; GTM positioning)

### 3. UPDATE: `.claude/.engineering/SESSION_MEMORY.md`
Add two new lessons:
- **Lesson: Code-Aware RAG Is the Wrong Track** — RAG for code files means code intelligence competing with Cursor/Copilot; KH's engineering customer (Sarah) needs document intelligence about engineering decisions, not AST traversal
- **Lesson: RAG Strategy Must Map to Customer Segment, Not Technology** — Every RAG pattern decision must start from "which customer segment does this improve?" not "is this a 2026 trend?"
...
- `artifact/RAG_TRENDS_2025_2026.md` — this is a general knowledge reference document,
  ...
  No changes needed — it serves its purpose as a research reference.
...
- The existing retrieval implementation (hybrid BM25+Dense+RRF, confidence gate, JSON sharding)
  is validated and correct. These changes only ADD strategic clarity, not change the code.
...
- `.claude/.engineering/DECISIONS.md` — UPDATE (append D.12–D.15)
- `.claude/.engineering/SESSION_MEMORY.md` — UPDATE (append 2 lessons)
- `.claude/.engineering/GLOSSARY.md` — UPDATE (append 5 terms)
- `.claude/.engineering/README.md` — UPDATE (1 line in reading order)
...
3. Open `SESSION_MEMORY.md` — the code RAG lesson should appear as a permanent warning
4. The four .engineering files together should make the answer to the user's original
   question self-evident to any future session that reads them
