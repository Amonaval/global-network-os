...
test('Phase 3 storage guard tolerates transient missing Storage size metadata without weakening tenant isolation',()=>{
 const migration=fs.readFileSync('supabase/migrations/096_phase3_storage_metadata_compatibility.sql','utf8');
 assert.match(migration,/bytes:=nullif\(new\.metadata->>'size',''\)::bigint/);
 assert.match(migration,/if bytes is null or bytes<=0 then return new/);
 assert.match(migration,/nid<>public\.current_network_id\(\) or not public\.is_network_member\(nid\)/);
 assert.match(migration,/file_size_limit/);
 assert.match(migration,/reconcile_network_media_usage/);
});
