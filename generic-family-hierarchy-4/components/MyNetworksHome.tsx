...
export default function MyNetworksHome({identity,onOpenNetwork,onAddNetwork,onExploreDemo,onSignOut,onDeleted}:{identity:TrustedPersonIdentity;onOpenNetwork:(membership:NetworkMembership)=>Promise<void>|void;onAddNetwork:()=>void;onExploreDemo?:(kind:NetworkVerticalKind)=>void;onSignOut?:()=>Promise<void>|void;onDeleted?:()=>Promise<void>|void}){
  ...
  {section==="overview"&&<>
   <section className="nx8-overview-grid">
    <div className="nx8-spaces-panel">
     ...
     {memberships.length>0?<div className="my-networks-grid nx6-networks-grid nx8-network-grid">{memberships.map(m=>{const def=getVerticalDefinition(m.network.verticalKind);return <div key={m.network.id} className="my-network-card-wrap"><button className={`my-network-card nx6-network-card ${m.isActive?"active":""}`} onClick={()=>onOpenNetwork(m)}><span className={`my-network-icon ${m.network.verticalKind}`}>{icon(m.network.verticalKind,24)}</span><span className="my-network-card-copy"><small>{def.displayName}</small><strong>{m.network.name}</strong><p>{outcome[m.network.verticalKind]}</p><span className="my-network-meta"><b>{m.role}</b>{m.isActive&&<em>{tr("CurrentTxt")}</em>}</span></span><ChevronRight size={20}/></button><div className="card-actions"><button className="btn small" disabled={lifecycleBusy} onClick={async()=>{if(!window.confirm(`Leave ${m.network.name}? This removes only your membership/access.`))return;setLifecycleBusy(true);try{await leaveNetwork(m.network.id);await onDeleted?.()}catch(e:any){window.alert(e.message||"Could not leave network.")}finally{setLifecycleBusy(false)}}}>Leave</button>{m.role==="owner"&&<><button className="btn small" disabled={lifecycleBusy} onClick={async()=>{const typed=window.prompt(`Type the exact network name to archive:
${m.network.name}`);if(typed!==m.network.name)return;if(!window.confirm(`Archive ${m.network.name}? Data and history are preserved and can be restored.`))return;setLifecycleBusy(true);try{await archiveNetwork(m.network.id,typed);await loadArchived();await onDeleted?.()}catch(e:any){window.alert(e.message||"Could not archive network.")}finally{setLifecycleBusy(false)}}}>Archive</button><button className="btn small danger-text my-network-delete" disabled={lifecycleBusy} onClick={async()=>{const typed=window.prompt(`Type the exact network name to delete:
${m.network.name}`);if(typed!==m.network.name)return;if(!window.confirm(`Permanently delete ${m.network.name}? This cannot be undone.`))return;setLifecycleBusy(true);try{await deleteOwnedNetworkPermanently(m.network.id,typed);await onDeleted?.()}catch(e:any){window.alert(e.message||"Could not delete network.")}finally{setLifecycleBusy(false)}}}>Delete</button></>}</div></div>})}</div>:<div className="card my-networks-empty"><Layers3/><div><h3>{tr("YourFirstNetworkStartsHereTxt")}</h3><p>{tr("CreateOrJoinOneOrUseTheTxt")}</p></div><button className="btn primary" onClick={onAddNetwork}>{tr("CreateOrJoinTxt")}</button></div>}
     {archivedNetworks.length>0&&<section className="card"><span className="warm-kicker">Archived</span><h3>Archived networks</h3><p>Archived networks preserve all data and history while normal member access is inactive.</p><div className="my-networks-grid">{archivedNetworks.map(n=><div key={n.networkId} className="my-network-card-wrap"><div className="my-network-card"><span className={`my-network-icon ${n.verticalKind}`}><Layers3 size={24}/></span><span className="my-network-card-copy"><small>{n.verticalKind}</small><strong>{n.name}</strong><span className="my-network-meta"><b>owner</b><em>Archived</em></span></span></div><div className="card-actions"><button className="btn small" disabled={lifecycleBusy} onClick={async()=>{setLifecycleBusy(true);try{await restoreNetwork(n.networkId);await loadArchived();await onDeleted?.()}catch(e:any){window.alert(e.message||"Could not restore network.")}finally{setLifecycleBusy(false)}}}>Restore</button><button className="btn small danger-text" disabled={lifecycleBusy} onClick={async()=>{const typed=window.prompt(`Type the exact network name to permanently delete:
${n.name}`);if(typed!==n.name)return;if(!window.confirm(`Permanently delete archived network ${n.name}? This cannot be undone.`))return;setLifecycleBusy(true);try{await deleteOwnedNetworkPermanently(n.networkId,typed);await loadArchived()}catch(e:any){window.alert(e.message||"Could not delete network.")}finally{setLifecycleBusy(false)}}}>Delete permanently</button></div></div>)}</div></section>}
    </div>
