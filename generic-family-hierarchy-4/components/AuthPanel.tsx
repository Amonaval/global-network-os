...
export default function AuthPanel({ onDone, initialMode="signin", onResetDone }: { onDone: () => void; initialMode?:AuthMode; onResetDone?:()=>void }) {
 const {t:tr}=useLanguage();
  ...
  return <div className="modal-overlay"><form className="modal auth-modal family-auth" role="dialog" aria-modal="true" aria-labelledby="auth-title" onSubmit={submit}>
    ...
    {mode!=="reset"&&<div className="field"><label htmlFor="auth-email">{tr("EmailAddressTxt")}</label><input id="auth-email" className="text-input" type="email" autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder={tr("YouExampleComTxt")} required /></div>}
    {(mode===tr("SigninTxt")||mode===tr("SignupTxt")||mode===tr("ResetTxt"))&&<div className="field"><label htmlFor="auth-password">{resetTitle?tr("NewPasswordTxt"):tr("PasswordTxt")}</label><div className="password-field"><input id="auth-password" className="text-input" type={showPassword?"text":"password"} autoComplete={resetTitle?"new-password":mode==="signin"?"current-password":"new-password"} minLength={8} value={password} onChange={e=>setPassword(e.target.value)} placeholder={tr("AtLeast8Characters2Txt")} required /><button type="button" className="password-toggle" aria-label={showPassword?"Hide password":"Show password"} onClick={()=>setShowPassword(v=>!v)}>{showPassword?<EyeOff size={17}/>:<Eye size={17}/>}</button></div></div>}
    ...
    <button className="btn primary auth-submit" disabled={busy}>{busy?tr("PleaseWaitTxt"):<>{mode==="signin"?tr("SignInTxt"):mode==="signup"?tr("CreateAccountTxt"):mode==="forgot"?tr("SendResetEmailTxt"):tr("SaveNewPasswordTxt")}{mode==="forgot"?<Mail size={16}/>:mode==="reset"?<KeyRound size={16}/>:<ArrowRight size={16}/>}</>}</button>
