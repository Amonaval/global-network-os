...
export default function NetworkAccountMenu({label="Me",subtitle,items}:{label?:string;subtitle?:string;items:NetworkAccountMenuItem[]}){
 ...
 return <div className="network-account-menu" ref={root}>
  <button className={`network-account-trigger ${open?"active":""}`} type="button" aria-haspopup="menu" aria-expanded={open} onClick={()=>setOpen(v=>!v)}><span className="network-account-avatar"><UserRound size={16}/></span><span className="network-account-copy"><b>{label}</b>{subtitle&&<small>{subtitle}</small>}</span><ChevronDown size={14}/></button>
