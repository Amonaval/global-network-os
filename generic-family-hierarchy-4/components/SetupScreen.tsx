    ...
    <main className={`onboarding-wrap onboarding-wrap-${path}`}>
      ...
      {path==="entry"&&<>
        ...
        <div className="alpha-entry-options">
          ...
          {shared&&<div className="playground-gallery"><div className="productized-create-head"><span className="warm-kicker"><PlayCircle size={13}/> {t("SafePlaygroundsTxt")}</span><b>{t("ExploreReleasedProductsTxt")}</b><small>{t("PlaygroundReadOnlyDescTxt")}</small></div><div className="playground-gallery-grid"><button onClick={onExploreDemo}><span className="playground-icon family"><TreePine/></span><b>{t("FamilyTxt")}</b><small>{tr("TreeMemoriesDescTxt")}</small><em>{t("OpenPlaygroundTxt")} <ArrowRight size={13}/></em></button>{onExploreAlumniDemo&&<button onClick={onExploreAlumniDemo}><span className="playground-icon alumni"><UsersRound/></span><b>{t("AlumniTxt")}</b><small>{tr("InstitutionExplorerDescTxt")}</small><em>{t("OpenPlaygroundTxt")} <ArrowRight size={13}/></em></button>}{onExploreProductizedDemo&&(["organization","business-trust","franchise","professional"] as ProductizedVerticalKind[]).map(kind=>{const pc=PRODUCTIZED_NETWORK_CONFIGS[kind];const Icon=kind==="organization"?Building2:kind==="business-trust"?Handshake:kind==="professional"?BriefcaseBusiness:Store;return <button key={kind} onClick={()=>onExploreProductizedDemo(kind)}><span className={`playground-icon ${kind}`}><Icon/></span><b>{kind==="professional"?t("ProfessionalShortTxt"):pc.shortLabel}</b><small>{kind==="professional"?t("ProfessionalHeroDescTxt"):pc.sampleDescription}</small><em>{t("OpenPlaygroundTxt")} <ArrowRight size={13}/></em></button>})}</div></div>}
