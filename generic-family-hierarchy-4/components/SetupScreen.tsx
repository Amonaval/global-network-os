...
export default function SetupScreen({ onCreate,onExploreDemo,onJoinCode,claimableProfiles=[],existingFamilies=[],onOpenFamily,onSignOut,onClaimProfile,shared,canSetup,approvalRequired=false,onOpenGuide,claimableAlumniProfiles=[],onClaimAlumniProfile,onCreateAlumni,onExploreAlumniDemo,networkInviteToken,onAcceptNetworkInvite,alumniInviteToken,onAcceptAlumniInvite,onCreateProductized,onExploreProductizedDemo,onJoinProductizedCode }: Props) {
 ...
 const {t:tr}=useLanguage();
  ...
  const [path,setPath]=useState<"entry"|"playgrounds"|"join"|"create"|"alumni"|"productized">("entry");
  ...
  const productizedKinds=["housing-society","family-association","association","organization","business-trust","franchise","professional"] as ProductizedVerticalKind[];
  const extraCreateKinds=productizedKinds.filter(kind=>kind!=="housing-society"&&kind!=="family-association"&&canCreateVertical(kind));
  const playgroundKinds=(["family","housing-society","family-association","association","alumni","organization","business-trust","franchise","professional"] as NetworkVerticalKind[]).filter(canPlayVertical);
  const verticalIcon=(kind:NetworkVerticalKind)=>kind==="family"?<TreePine/>:kind==="housing-society"?<Building2/>:(kind==="association"||kind==="family-association"||kind==="alumni")?<UsersRound/>:kind==="organization"?<Building2/>:kind==="business-trust"?<Handshake/>:kind==="professional"?<BriefcaseBusiness/>:<Store/>;
  const startProductized=(kind:ProductizedVerticalKind)=>{setProductizedKind(kind);setProductizedName("");setProductizedContext("");setProductizedDescription("");setProductizedStep(1);setPath("productized")};
  const openPlayground=(kind:NetworkVerticalKind)=>{if(kind==="family"){onExploreDemo();return;}if(kind==="alumni"){onExploreAlumniDemo?.();return;}if(productizedKinds.includes(kind as ProductizedVerticalKind))onExploreProductizedDemo?.(kind as ProductizedVerticalKind)};
  ...
  return <div className="family-onboarding alpha-entry" data-testid="qa-setup-shell">
    ...
    <main className={`onboarding-wrap onboarding-wrap-${path}`} data-testid="qa-start-flow">
      <section className="onboarding-story"><span className="warm-kicker"><Heart size={13} fill="currentColor"/> {tr("SetupMadeForTxt")}</span><h1>{path==="entry"?tr("ShowcaseSetupTitleTxt"):path==="playgrounds"?tr("ShowcaseExploreTitleTxt"):path==="join"?t("JoinYourFamilyTxt"):path==="productized"?PRODUCTIZED_NETWORK_CONFIGS[productizedKind].createTitle:t("CreateFamilySpaceTxt")}</h1><p>{path==="entry"?tr("ShowcaseSetupDescTxt"):path==="playgrounds"?tr("ShowcaseExploreDescTxt"):path==="join"?t("SetupJoinDescTxt"):path==="productized"?PRODUCTIZED_NETWORK_CONFIGS[productizedKind].createDescription:t("SetupCreateDescTxt")}</p></section>
      ...
      {path==="entry"&&<>
        ...
        <div className="showcase-entry-head"><span className="warm-kicker"><Sparkles size={13}/> {tr("ShowcaseChoosePathKickerTxt")}</span><h3>{tr("ShowcaseChoosePathTitleTxt")}</h3><p>{tr("ShowcaseChoosePathDescTxt")}</p></div>
        <div className="showcase-choice-grid">
          {canCreateVertical("family")&&<button data-testid="qa-entry-create-family" className="showcase-choice-card family" onClick={()=>setPath("create")}><span className="showcase-choice-icon"><TreePine/></span><span><small>{tr("ShowcaseCreateTxt")}</small><b>{tr("ShowcaseFamilyTitleTxt")}</b><p>{tr("ShowcaseFamilyDescTxt")}</p><em>{approvalRequired?tr("RequestCreateTxt"):tr("CreateNowTxt")} <ArrowRight size={15}/></em></span></button>}
          {shared&&onCreateProductized&&canCreateVertical("family-association")&&<button data-testid="qa-entry-create-family-association" className="showcase-choice-card community" onClick={()=>startProductized("family-association")}><span className="showcase-choice-icon"><UsersRound/></span><span><small>{tr("ShowcaseCreateTxt")}</small><b>{tr("ShowcaseCommunityTitleTxt")}</b><p>{tr("ShowcaseCommunityDescTxt")}</p><em>{tr("CreateNetworkTxt")} <ArrowRight size={15}/></em></span></button>}
          {shared&&onCreateProductized&&canCreateVertical("housing-society")&&<button data-testid="qa-entry-create-housing-society" className="showcase-choice-card residential" onClick={()=>startProductized("housing-society")}><span className="showcase-choice-icon"><Building2/></span><span><small>{tr("ShowcaseCreateTxt")}</small><b>{tr("ShowcaseResidentialTitleTxt")}</b><p>{tr("ShowcaseResidentialDescTxt")}</p><em>{tr("CreateNetworkTxt")} <ArrowRight size={15}/></em></span></button>}
          {shared&&playgroundKinds.length>0&&<button data-testid="qa-entry-explore-playground" className="showcase-choice-card explore" onClick={()=>setPath("playgrounds")}><span className="showcase-choice-icon"><PlayCircle/></span><span><small>{tr("ShowcaseExploreKickerTxt")}</small><b>{tr("ExplorePlaygroundTxt")}</b><p>{tr("ShowcaseExploreChoiceDescTxt")}</p><em>{tr("ExploreTxt")} <ArrowRight size={15}/></em></span></button>}
        </div>
        <div className="showcase-entry-secondary">
          <button data-testid="qa-entry-join-family" className="btn" onClick={()=>setPath("join")}><UsersRound size={15}/> {tr("JoinExistingFamilyTxt")}</button>
          {(canCreateVertical("alumni")||extraCreateKinds.length>0)&&<details className="showcase-more-types"><summary>{tr("ShowcaseMoreNetworkTypesTxt")}</summary><div className="showcase-more-grid">{shared&&onCreateAlumni&&canCreateVertical("alumni")&&<button onClick={()=>setPath("alumni")}><UsersRound/><span><b>{tr("CreateAlumniNetworkTxt")}</b><small>{tr("BuildAlumniDescTxt")}</small></span></button>}{extraCreateKinds.map(kind=>{const pc=PRODUCTIZED_NETWORK_CONFIGS[kind];return <button key={kind} onClick={()=>startProductized(kind)}>{verticalIcon(kind)}<span><b>{kind==="professional"?t("ProfessionalNetworkTxt"):pc.label}</b><small>{kind==="professional"?t("ProfessionalCreateDescTxt"):pc.createDescription}</small></span></button>})}</div></details>}
          {onJoinProductizedCode&&<details className="showcase-join-code"><summary>{t("HaveJoinCodeTxt")}</summary><div className="productized-join-row"><div><KeyRound size={17}/><span><b>{t("HaveJoinCodeTxt")}</b><small>{t("JoinCodeDescTxt")}</small></span></div><input className="text-input" value={productizedJoinCode} onChange={e=>setProductizedJoinCode(e.target.value.toUpperCase())} placeholder={t("NetworkCodeTxt")} maxLength={12}/><button className="btn" disabled={busy||productizedJoinCode.trim().length<4} onClick={async()=>{if(!onJoinProductizedCode)return;setBusy(true);setError("");try{await onJoinProductizedCode(productizedJoinCode.trim())}catch(e:any){setError(e.message||tr("CouldNotJoinThatNetworkTxt"))}finally{setBusy(false)}}}>{t("JoinNetworkTxt")}</button></div></details>}
        </div>
      ...
      {path==="playgrounds"&&<>
        ...
        <div className="setup-heading showcase-playground-heading"><div className="brand-mark"><PlayCircle size={24}/></div><div><span className="setup-eyebrow">{tr("ShowcaseExploreKickerTxt")}</span><h2>{tr("ShowcaseExploreTitleTxt")}</h2></div></div>
        <p className="setup-intro">{tr("ShowcaseExploreDescTxt")}</p>
        <div className="showcase-playground-grid">{playgroundKinds.map(kind=>{const def=getVerticalDefinition(kind);const description=kind==="family"?tr("ShowcaseFamilyDescTxt"):kind==="family-association"?tr("ShowcaseCommunityDescTxt"):kind==="housing-society"?tr("ShowcaseResidentialDescTxt"):kind==="professional"?t("ProfessionalHeroDescTxt"):kind==="alumni"?tr("InstitutionExplorerDescTxt"):PRODUCTIZED_NETWORK_CONFIGS[kind as ProductizedVerticalKind]?.sampleDescription||def.displayName;return <button data-testid={`qa-playground-${kind}`} key={kind} onClick={()=>openPlayground(kind)}><span className={`playground-icon ${kind}`}>{verticalIcon(kind)}</span><span><small>{tr("ShowcaseSampleTxt")}</small><b>{def.displayName}</b><p>{description}</p><em>{t("OpenPlaygroundTxt")} <ArrowRight size={14}/></em></span></button>})}</div>
        <div className="notice success-notice"><ShieldCheck size={15}/><span><b>{tr("ShowcaseReadOnlyTxt")}</b> {tr("PlaygroundIsReadOnlyAndTemporaryNothingTxt")}</span></div>
      </>}
      {path==="join"&&<>
        <button className="setup-back" onClick={()=>{setPath("entry");setError("")}}><ArrowLeft size={15}/> {t("BackTxt")}</button>
        ...
        <button className="btn demo-inline" onClick={()=>openPlayground("family")}><PlayCircle size={16}/> {tr("ExploreSampleInsteadTxt")}</button>
      </>}
      {path==="alumni"&&<>
        <button className="setup-back" onClick={()=>{setPath("entry");setError("")}}><ArrowLeft size={15}/> {t("BackTxt")}</button>
        {alumniStep===1?<>
