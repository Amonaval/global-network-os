    ...
    <main className={`onboarding-wrap onboarding-wrap-${path}`}>
      ...
      {path==="entry"&&<>
        ...
        <div className="alpha-entry-options">
          ...
          <button className="alpha-entry-option" onClick={()=>setPath("create")}><span><TreePine/></span><b>{t("CreateMyFamilyTxt")}</b><small>{t("CreateMyFamilyDescTxt")}</small><em>{approvalRequired?tr("RequestCreateTxt"):tr("CreateNowTxt")} <ArrowRight size={15}/></em></button>
          {shared&&onCreateAlumni&&<button className="alpha-entry-option" onClick={()=>setPath("alumni")}><span><UsersRound/></span><b>{tr("CreateAlumniNetworkTxt")}</b><small>{tr("BuildAlumniDescTxt")}</small><em>{tr("CreateAlumniTxt")}{" "}<ArrowRight size={15}/></em></button>}
        </div>
          {shared&&<div className="playground-gallery"><div className="productized-create-head"><span className="warm-kicker"><PlayCircle size={13}/> {t("SafePlaygroundsTxt")}</span><b>{t("ExploreReleasedProductsTxt")}</b><small>{t("PlaygroundReadOnlyDescTxt")}</small></div><div className="playground-gallery-grid"><button onClick={onExploreDemo}><span className="playground-icon family"><TreePine/></span><b>{t("FamilyTxt")}</b><small>{tr("TreeMemoriesDescTxt")}</small><em>{t("OpenPlaygroundTxt")} <ArrowRight size={13}/></em></button>{onExploreAlumniDemo&&<button onClick={onExploreAlumniDemo}><span className="playground-icon alumni"><UsersRound/></span><b>{t("AlumniTxt")}</b><small>{tr("InstitutionExplorerDescTxt")}</small><em>{t("OpenPlaygroundTxt")} <ArrowRight size={13}/></em></button>}{onExploreProductizedDemo&&(["family-association","association","organization","business-trust","franchise","professional"] as ProductizedVerticalKind[]).map(kind=>{const pc=PRODUCTIZED_NETWORK_CONFIGS[kind];const Icon=(kind==="association"||kind==="family-association")?UsersRound:kind==="organization"?Building2:kind==="business-trust"?Handshake:kind==="professional"?BriefcaseBusiness:Store;return <button key={kind} onClick={()=>onExploreProductizedDemo(kind)}><span className={`playground-icon ${kind}`}><Icon/></span><b>{kind==="professional"?t("ProfessionalShortTxt"):pc.shortLabel}</b><small>{kind==="professional"?t("ProfessionalHeroDescTxt"):pc.sampleDescription}</small><em>{t("OpenPlaygroundTxt")} <ArrowRight size={13}/></em></button>})}</div></div>}
          {shared&&onCreateProductized&&<div className="productized-create-strip"><div className="productized-create-head"><span className="warm-kicker">{t("NetworkOsProductsTxt")}</span><b>{t("CreateAnotherNetworkTxt")}</b><small>{t("SharedCapabilitiesDescTxt")}</small></div><div className="productized-create-grid">{(["family-association","association","organization","business-trust","franchise","professional"] as ProductizedVerticalKind[]).map(kind=>{const pc=PRODUCTIZED_NETWORK_CONFIGS[kind];const Icon=(kind==="association"||kind==="family-association")?UsersRound:kind==="organization"?Building2:kind==="business-trust"?Handshake:kind==="professional"?BriefcaseBusiness:Store;return <div className="productized-create-card" key={kind}><button onClick={()=>{setProductizedKind(kind);setProductizedName("");setProductizedContext("");setProductizedDescription("");setPath("productized")}}><span><Icon/></span><b>{kind==="professional"?t("ProfessionalNetworkTxt"):pc.label}</b><small>{kind==="professional"?t("ProfessionalCreateDescTxt"):pc.createDescription}</small><em>{t("CreateNetworkTxt")} <ArrowRight size={14}/></em></button>{onExploreProductizedDemo&&<button className="productized-demo-link" onClick={()=>onExploreProductizedDemo(kind)}><PlayCircle size={14}/> {t("TrySampleTxt")}</button>}</div>})}</div>{onJoinProductizedCode&&<div className="productized-join-row"><div><KeyRound size={17}/><span><b>{t("HaveJoinCodeTxt")}</b><small>{t("JoinCodeDescTxt")}</small></span></div><input className="text-input" value={productizedJoinCode} onChange={e=>setProductizedJoinCode(e.target.value.toUpperCase())} placeholder={t("NetworkCodeTxt")} maxLength={12}/><button className="btn" disabled={busy||productizedJoinCode.trim().length<4} onClick={async()=>{if(!onJoinProductizedCode)return;setBusy(true);setError("");try{await onJoinProductizedCode(productizedJoinCode.trim())}catch(e:any){setError(e.message||tr("CouldNotJoinThatNetworkTxt"))}finally{setBusy(false)}}}>{t("JoinNetworkTxt")}</button></div>}</div>}
      ...
      {path==="create"&&<>
        ...
        {step===1?<>
          ...
          <div className="family-start-options">
            <button className="family-start-card recommended" disabled={busy||!canSetup} onClick={()=>create("empty")}><span className="start-icon"><UsersRound/></span><span className="recommended-pill">{t("RecommendedLowestEffortTxt")}</span><strong>{t("BuildTogetherRelativesTxt")}</strong><small>{tr("CreateTheFamilyFirstThenShareSimpleTxt")}</small><b>{approvalRequired?tr("RequestFamilyTxt"):tr("CreateFamilyAndCollectBranchesTxt")} <ArrowRight size={15}/></b></button>
            <button className="family-start-card" disabled={busy||!canSetup} onClick={()=>setShowImport(true)}><span className="start-icon"><FileSpreadsheet/></span><strong>{t("UploadExcelCsvTxt")}</strong><small>{tr("BestWhenYouAlreadyHaveAFamilyTxt")}</small><b>{tr("SetupOpenExcelTxt")}{" "}<ArrowRight size={15}/></b></button>
            <button className="family-start-card" disabled={busy||!canSetup} onClick={()=>create("empty")}><span className="start-icon"><Sparkles/></span><strong>{t("StartMyselfTxt")}</strong><small>{tr("CreateFamilyNowTxt")}</small><b>{approvalRequired?tr("RequestFamilyTxt"):tr("CreateFamilyNow2Txt")} <ArrowRight size={15}/></b></button>
          </div>
        </>}
      </>}
