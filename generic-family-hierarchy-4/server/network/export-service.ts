...
async function listAll(client:ReturnType<typeof createClient>,bucket:string,prefix:string){const out:string[]=[];const queue=[prefix];while(queue.length){const dir=queue.shift()!;let offset=0;for(;;){const {data,error}=await client.storage.from(bucket).list(dir,{limit:100,offset});if(error)throw error;const rows=(data||[]) as Entry[];for(const row of rows){const path=dir?`${dir}/${row.name}`:row.name;if(!row.id&&row.metadata==null)queue.push(path);else out.push(path)}if(rows.length<100)break;offset+=rows.length}}return out}
