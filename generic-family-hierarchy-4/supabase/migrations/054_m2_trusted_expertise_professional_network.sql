-- M2 — Trusted Expertise & Professional Network
-- Additive after 053. Activates "professional" as the sixth Network OS vertical.

-- 1) Expand vertical whitelists without changing existing tenant semantics.
...
alter table public.networks add constraint networks_vertical_kind_check
 check (vertical_kind in ('family','alumni','organization','business-trust','franchise','professional'));
...
alter table public.productized_network_settings add constraint productized_network_settings_template_id_check
 check(template_id in ('organization','business-trust','franchise','professional'));

-- 2) Productized-template validation helpers.
create or replace function public.g8_productized_vertical(p_kind text) returns boolean language sql immutable as $$
 select p_kind in ('organization','business-trust','franchise','professional');
$$;
...
-- 3) Seed professional dimensions/projections while preserving the existing three productized templates.
...
begin
 if p_kind='organization' then
  ...
  perform public.g7_ensure_dimension(p_network,'profession','Profession',10);
  perform public.g7_ensure_dimension(p_network,'specialty','Specialty / Expertise',20);
  perform public.g7_ensure_dimension(p_network,'industry','Industry',30);
  perform public.g7_ensure_dimension(p_network,'service','Service',40);
  perform public.g7_ensure_dimension(p_network,'country','Country',50);
  perform public.g7_ensure_dimension(p_network,'city','City',60);
  perform public.g7_ensure_dimension(p_network,'credential','Credential / Qualification',70);
  insert into public.network_projections(network_id,projection_key,label,levels,is_default,sort_order) values
   (p_network,'expertise-location','Expertise → Country → City',array['specialty','country','city'],true,10),
   (p_network,'profession-service','Profession → Service → Specialty',array['profession','service','specialty'],false,20),
   (p_network,'industry-expertise','Industry → Expertise → City',array['industry','specialty','city'],false,30)
  on conflict(network_id,projection_key) do update set label=excluded.label,levels=excluded.levels,is_default=excluded.is_default,sort_order=excluded.sort_order;
 end if;
end $$;

-- 4) Creation semantics for the new vertical.
...
-- 5) Launch Control recognizes the new vertical.
...
begin
 ...
 if p_vertical_kind not in ('family','alumni','organization','business-trust','franchise','professional') then raise exception 'Unknown vertical.' using errcode='22023';end if;
...
-- 6) Feature catalog defaults: core/discovery/community/connections/contributions released; intelligence remains test.
insert into public.platform_feature_flags(feature_key,bundle_key,rollout_state,pilot_network_ids,vertical_kind) values
 ('professional.core.home','core','released','{}'::uuid[],'professional'),
 ('professional.shared.intelligence','intelligence','test','{}'::uuid[],'professional'),
 ('professional.shared.explorer','discover','released','{}'::uuid[],'professional'),
 ('professional.core.directory','discover','released','{}'::uuid[],'professional'),
 ('professional.shared.community','community','released','{}'::uuid[],'professional'),
 ('professional.shared.places','discover','released','{}'::uuid[],'professional'),
 ('professional.core.connections','connect','released','{}'::uuid[],'professional'),
 ('professional.shared.contribute','contribute','released','{}'::uuid[],'professional'),
 ('professional.admin.manage','admin','released','{}'::uuid[],'professional'),
 ('professional.admin.import','admin','released','{}'::uuid[],'professional')
...
from public.platform_feature_flags where vertical_kind='professional'
