...
const errorDetails=(error)=>error?{message:error.message||null,name:error.name||null,status:error.status||error.statusCode||null,code:error.code||null,error:error.error||null,details:error.details||null,hint:error.hint||null,raw:JSON.parse(JSON.stringify(error))}:null;
...
try{
 const upB=await B.client.storage.from('profile-photos').upload(storagePath,png,{contentType:'image/png',upsert:false});storageCreated=!upB.error;assert(storageCreated,'Tenant B can upload to its own tenant-prefixed storage path',{error:errorDetails(upB.error)});if(upB.error)console.error('[RLS storage upload diagnostic]',JSON.stringify(errorDetails(upB.error),null,2));
 if(storageCreated){
...
const failed=checks.filter(x=>x.status==='failed');if(failed.length)console.error('[RLS failed checks]',JSON.stringify(failed,null,2));writeJson('qa-results/security/rls-adversarial.json',{generatedAt:new Date().toISOString(),status:failed.length?'failed':'passed',summary:{checks:checks.length,failed:failed.length},checks});console.log(`RLS adversarial ${failed.length?'FAIL':'PASS'} (${checks.length} checks, ${failed.length} failed)`);if(failed.length)process.exit(1);
