...
test.describe.serial('Phase-2 representative capability certification',()=>{
  ...
  test('API + lifecycle: create → readback → bootstrap → update → delete/purge → cleanup proof',async({request})=>{
    ...
    try{
      ...
      const inserted=await a.client.from('network_entities').select('id,label,metadata').eq('network_id',networkId).eq('label',label).single();
      expect(inserted.error).toBeNull();
      expect(inserted.data?.metadata?.state).toBe('created');

      const updatedLabel=`${label} Updated`;
      const updated=await a.client.rpc('upsert_productized_network_entity',{p_entity_id:inserted.data!.id,p_kind:'person',p_label:updatedLabel,p_metadata:{qaPurpose:'phase2-lifecycle',state:'updated'},p_affiliations:{},p_visibility:'members'});
      expect(updated.error).toBeNull();
      const verifyUpdate=await a.client.from('network_entities').select('label,metadata').eq('network_id',networkId).eq('id',inserted.data!.id).single();
      expect(verifyUpdate.error).toBeNull();
      expect(verifyUpdate.data?.label).toBe(updatedLabel);
      expect(verifyUpdate.data?.metadata?.state).toBe('updated');

      const purge=await request.post(`/api/v1/networks/${networkId}/purge`,{headers:{authorization:`Bearer ${a.token}`},data:{confirmName:networkName}});
      expect(purge.status()).toBe(200);purged=true;
      ...
      if(!purged){try{await request.post(`/api/v1/networks/${networkId}/purge`,{headers:{authorization:`Bearer ${a.token}`},data:{confirmName:networkName}})}catch{}}
